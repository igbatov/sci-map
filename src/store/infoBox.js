...
export default {
  ...
  actions: {
    async [FetchWiki]({commit, getters, rootGetters}, nodeId) {
      ...
      const wikiPage = node.link.substr("https://en.wikipedia.org/wiki/".length);
    ...
    [ParseSections]({commit, getters, rootGetters}, nodeId) {
      ...
      const noteList = htmlToText.fromString(node.note, {
        wordwrap: false,
      ...
      noteList.forEach((note) => {
        ...
        if ([SECTION_ME, SECTION_CP].indexOf(groups[1]) !== -1) {
          ...
          if ([TYPE_COURSE, TYPE_SCIPOP, TYPE_TEXTBOOK].indexOf(groups[2])) {
            sections[groups[1]].push({type: groups[2], votes:groups[3], name:groups[4], link:groups[5]})
          }
        }
      })
