...
export const store = {
  ...
  actions: {
    [actions.getNodeContent]({state}: { state: State }, nodeID: string): NodeContent {
      return state.nodeContents[nodeID]
    }
  },
  mutations: {
    [mutations.SET_CONTENTS](state: State, nodeContents: Record<string, NodeContent>) {
      state.nodeContents = nodeContents
    },
    [mutations.SET_NODE_WIKIPEDIA](state: State, v: {nodeID: string, wikipedia: string}) {
      state.nodeContents[v.nodeID].wikipedia = v.wikipedia
    },
    [mutations.SET_NODE_COMMENT](state: State, v: {nodeID: string, comment: string}) {
      state.nodeContents[v.nodeID].comment = v.comment
    },
    [mutations.ADD_TO_NODE_RESOURCES](state: State, v: {rr: ResourceRating, nodeID: string}) {
      state.nodeContents[v.nodeID].resources[v.rr.resourceID] = v.rr
    },
    [mutations.RATE_NODE_RESOURCE](state: State, v: {nodeID: string, resourceID: string, rating: number}) {
      state.nodeContents[v.nodeID].resources[v.resourceID].rate = v.rating
    },
    [mutations.HIDE_NODE_RESOURCE](state: State, v: {nodeID: string, resourceID: string, hide: boolean}) {
      state.nodeContents[v.nodeID].resources[v.resourceID].hide = v.hide
    },
    [mutations.SET_NODE_RESOURCE_COMMENT](state: State, v: {nodeID: string, resourceID: string, comment: string}) {
