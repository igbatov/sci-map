...
export function convertDBMapToTree(
  ...
  while (stack.length > 0) {
    ...
    const children: Tree[] = [];
    const childrenCenters = dbNodes[node.id].children.reduce(
      (obj: Record<string, Point>, id) => {
    ...
    const [childrenBorders, err2] = getVoronoiCellRecords(
      borders[node.id],
      denormalizedPositions!
    );
    if (err2 !== null) {
      return [null, err2];
    }
    for (const childID of dbNodes[node.id].children) {
      if (!childrenBorders[childID]) {
        return [
          null,
          NewErrorKV("Cannot find childID in childrenBorders", {
            childID,
            childrenBorders,
