<template>
  <div v-for="precondition of preconditions" class="p-grid" :key="precondition.id">
    <div class="p-col-12">
      <div class="p-grid">
        <div class="p-col-11">
          {{ precondition.title }}
...
export default {
  ...
  setup(props: { nodeId: string }) {
    ...
    watchEffect(() => {
      preconditions.value = []
      if (props.nodeId && store.state.precondition.preconditions[props.nodeId]) {
    ...
    return {
      preconditions,
      remove: (id: string) => {
        store.dispatch(`precondition/${preconditionActions.RemovePrecondition}`, {
          nodeId: props.nodeId,
          preconditionId: id,
        });
