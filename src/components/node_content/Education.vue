<template>
  ...
  <div v-if="newFormShow" class="p-grid">
    <div class="p-col-12">
      <div class="p-grid">
        <div class="p-col-12">
          <SelectButton v-model="selectedType" :options="typeOptions" optionLabel="name" optionValue="code"/>
        </div>
      </div>
      <div v-for="field of types[selectedType].fields" class="p-field p-grid" :key="Object.keys(field)[0]">
        <label :for="Object.keys(field)[0]" class="p-col-3 p-mb-0">{{Object.values(field)[0]}}</label>
        <div class="p-col-9">
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      typeOptions: Object.keys(types).map(key => ({name: types[key].name, code: key})),
      types
    }
  }
}
